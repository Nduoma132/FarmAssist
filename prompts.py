...
def build_prompt(user_name, user_region, user_input, chat_history=None):
    region_part = f"{user_region}" if user_region else ""
    chat_memory = ""
    ...
    prompt = f"""
        ...
        You are having an ongoing conversation with a farmer named {user_name} in {region_part}. You already know their name and region, so there is no need to ask again.
        ...
        2. Tailor your advice to farming conditions in {region_part} in Nigeria only.
